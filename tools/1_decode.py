...
# krds: see https://www.mobileread.com/forums/showthread.php?t=322172
...
def check_krds(start_dir):
    for dir_name, subdir_list, file_list in os.walk(start_dir):
        for fname in file_list:
            for ftype in KRDS_FILETYPES:
                if fname.endswith(f'.{ftype}'):
                    print(f'Found file: {dir_name} {fname} {ftype}')

                    fpath = os.path.join(dir_name, fname)
                    with io.open(fpath, "rb") as infile:
                        binary_data = infile.read()

                    decoded_objects = KindleReaderDataStore(logging, binary_data).deserialize()

                    print(json.dumps(decoded_objects, indent=4, separators=(",", ": ")))

        for sdir in subdir_list:
            check_krds(os.path.join(dir_name, sdir))


def check_fmcache(fname):
    conn = sqlite3.connect(fname)

    cur = conn.cursor()
    for row in cur.execute('SELECT * FROM reading_sessions'):
        print(row)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('usage: ./1_decode.py /path/to/data')
        sys.exit(1)
    data_dir = sys.argv[1]
    if not os.path.isdir(data_dir):
        print(f'{data_dir} not a directory?')
        sys.exit(1)

    check_krds(data_dir)

    fmcache = os.path.join(data_dir, 'fmcache.db')
    if not os.path.isfile(fmcache):
        print(f'{fmcache} file not found')
        sys.exit(1)

    check_fmcache(fmcache)
